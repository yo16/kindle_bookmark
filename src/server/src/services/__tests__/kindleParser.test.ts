...
const TEST_PATHS = {
  XML_SAMPLE: path.resolve(__dirname, '../../../../../sample_file/KindleSyncMetadataCache.xml'),
...
describe('KindleXMLParser', () => {
  ...
  describe('KindleXMLParser初期化', () => {
    ...
    it('USERPROFILE環境変数が未設定の場合はエラー', () => {
      ...
      expect(() => new KindleXMLParser()).toThrow('Windows環境でのみ動作します');
  ...
  describe('parseXMLFile', () => {
    it('サンプルファイルを正常に解析する', async () => {
      // サンプルファイルの存在確認
      const fileExists = await fs.access(TEST_PATHS.XML_SAMPLE)
        .then(() => true)
        .catch(() => false);

      if (!fileExists) {
        console.warn('サンプルファイルが見つかりません。テストをスキップします。');
        return;
      }
      ...
      console.log(`サンプルファイル解析結果: ${result.books.length}冊、処理時間: ${result.statistics.processingTimeMs}ms`);
    }, 10000); // 10秒のタイムアウト
    ...
    it('パフォーマンス目標を満たす', async () => {
      ...
      if (!fileExists) {
        console.warn('サンプルファイルが見つかりません。パフォーマンステストをスキップします。');
        return;
      }
      ...
      console.log(`パフォーマンステスト結果: ${processingTime}ms（目標: 5000ms以内）`);
    }, 10000);

    it('存在しないファイルでエラー', async () => {
      const nonExistentPath = path.join(process.env.USERPROFILE!, 'AppData', 'Local', 'Amazon', 'Kindle', 'Cache', 'nonexistent.xml');

      await expect(parser.parseXMLFile(nonExistentPath))
        .rejects
    ...
    it('不正なファイルパス（パストラバーサル）でエラー', async () => {
      ...
      await expect(parser.parseXMLFile(maliciousPath))
        .rejects
        .toThrow('セキュリティ違反');
    });
    ...
    it('Kindleキャッシュ外のファイルでエラー', async () => {
      ...
      await expect(parser.parseXMLFile(outsidePath))
        .rejects
        .toThrow('セキュリティ違反');
    });

    it('XML以外のファイルでエラー', async () => {
      const nonXmlPath = path.join(process.env.USERPROFILE!, 'AppData', 'Local', 'Amazon', 'Kindle', 'Cache', 'test.txt');

      await expect(parser.parseXMLFile(nonXmlPath))
        .rejects
        .toThrow('XMLファイルのみ処理可能です');
    });

    it('大容量ファイルでエラー', async () => {
      // 10MBを超える仮想的なパス
      const largePath = path.join(process.env.USERPROFILE!, 'AppData', 'Local', 'Amazon', 'Kindle', 'Cache', 'large.xml');
      ...
      await expect(parser.parseXMLFile(largePath))
        .rejects
        .toThrow('XMLファイルが大きすぎます');
  ...
  describe('書籍メタデータ検証', () => {
    it('必須フィールドが正しく抽出される', async () => {
      ...
      if (!fileExists) {
        console.warn('サンプルファイルが見つかりません。メタデータテストをスキップします。');
        return;
      }
      ...
      sampleBooks.forEach((book, index) => {
        console.log(`  ${index + 1}. ${book.title} / ${book.author} (${book.asin})`);
      });
    }, 10000);
    ...
    it('日付フィールドが正しく処理される', async () => {
      ...
      const result = await parser.parseXMLFile(TEST_PATHS.XML_SAMPLE);
      const booksWithDates = result.books.filter(book =>
        book.publicationDate || book.purchaseDate
      );
    ...
    it('著者情報が正しく処理される', async () => {
      ...
      const result = await parser.parseXMLFile(TEST_PATHS.XML_SAMPLE);
      const booksWithPronunciation = result.books.filter(book =>
        book.authorPronunciation
      );
    ...
    it('書籍の取得方法が正しく分類される', async () => {
      ...
      const purchaseBooks = booksWithOrigin.filter(book => book.originType === 'Purchase');
      const kindleUnlimitedBooks = booksWithOrigin.filter(book => book.originType === 'KindleUnlimited');

      console.log(`取得方法統計: Purchase=${purchaseBooks.length}, KindleUnlimited=${kindleUnlimitedBooks.length}`);

      expect(purchaseBooks.length + kindleUnlimitedBooks.length).toBe(booksWithOrigin.length);
    });
  });
  ...
  describe('エラーハンドリング', () => {
    it('破損したXMLファイルを適切に処理する', async () => {
      // 無効なXMLテストファイルを作成
      const invalidXmlPath = path.join(process.env.USERPROFILE!, 'AppData', 'Local', 'Amazon', 'Kindle', 'Cache', 'invalid_test.xml');
      const invalidXmlContent = '<?xml version="1.0"?><response><unclosed-tag></response>';

      try {
        await fs.writeFile(invalidXmlPath, invalidXmlContent, 'utf8');

        await expect(parser.parseXMLFile(invalidXmlPath))
          .rejects
          .toThrow();
    ...
    it('空のXMLファイルを適切に処理する', async () => {
      const emptyXmlPath = path.join(process.env.USERPROFILE!, 'AppData', 'Local', 'Amazon', 'Kindle', 'Cache', 'empty_test.xml');

      try {
        await fs.writeFile(emptyXmlPath, '', 'utf8');

        await expect(parser.parseXMLFile(emptyXmlPath))
          .rejects
          .toThrow('XMLファイルが空です');
